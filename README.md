...
|---|---|---|
| **LuxVolt voice receptionist** — [voice.luxvolt.com.br](https://voice.luxvolt.com.br) | Retell AI phone agent for a solar / EV-charger installer. Looks up the caller, qualifies the job, books a site visit from real availability, opens support tickets, escalates emergencies to a human. Every tool call is journaled before the agent speaks. Agent and LLM are provisioned from versioned JSON through the Retell API, not clicked together. | Retell AI · Node 24 · Hono · `node:sqlite` · Docker · Traefik |
